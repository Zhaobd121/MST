...
public:
    ...
    std::vector<Cluster> exactClustering(const std::string &directory_name, int mu, const double& eps){
        FCFEdges = read_edges("index/"+directory_name+"_"+std::to_string(mu)+"/"+"newFCI.txt", eps);
        NCIEdges = read_edges("index/"+directory_name+"_"+std::to_string(mu)+"/"+"newNCI.txt", eps);
    ...
    std::vector<Cluster> densityClustering(const std::string &directory_name, int mu, const double& eps){
        FCFEdges = std::move(read_edges("index/"+directory_name+"_"+std::to_string(mu)+"/"+"newFCI.txt", eps));
        NCIEdges = std::move(read_edges("index/"+directory_name+"_"+std::to_string(mu)+"/"+"newDNCI.txt", eps));
        std::ofstream log_file("clustering_log.txt", std::ios::app);
        Timer t;
        std::vector<Cluster> result;
        DisJointSet clusters = DisJointSet(n1);
